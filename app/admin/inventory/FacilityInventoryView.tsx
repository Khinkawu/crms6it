    ...
    return (
        <div className="space-y-8">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div className="grid grid-cols-2 sm:flex sm:flex-row gap-3 w-full md:w-auto md:ml-auto">
                    ...
                    <button
                        ...
                        className={`px-4 py-2 border font-bold rounded-xl flex items-center justify-center gap-2 transition-all ${isSelectionMode ? 'bg-gray-900 dark:bg-white text-white dark:text-gray-900 border-gray-900 dark:border-white' : 'bg-white dark:bg-card border-border text-text hover:bg-gray-50 dark:hover:bg-gray-800'}`}
                    >
                    ...
                    {isSelectionMode && (
                        <>
                            <button
                                ...
                                className="px-4 py-2 bg-gray-700 text-white font-bold rounded-xl hover:bg-gray-600 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                            >
                            ...
                            <button
                                ...
                                className="px-4 py-2 bg-gray-600 text-white font-bold rounded-xl hover:bg-gray-500 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                            >
                    ...
                    {(role === 'admin' || role === 'facility_technician') && (
                        <button
                            ...
                            className="col-span-2 sm:col-span-1 px-4 py-2 bg-gray-900 dark:bg-white text-white dark:text-gray-900 font-bold rounded-xl hover:bg-gray-700 dark:hover:bg-gray-100 transition-colors flex items-center justify-center gap-2"
                        >
            ...
            <div className="max-w-7xl mx-auto">
                            ...
                            >
                                <div className="relative aspect-[4/3] bg-gray-100 dark:bg-gray-800 overflow-hidden" onClick={(e) => { if (isSelectionMode) { e.stopPropagation(); handleSelectProduct(product.id!); } }}>
                                    ...
                                    <div className="absolute top-2 right-2">
                                        <span className={`px-2 py-1 rounded-full text-xs font-bold shadow-sm ${product.status === 'available' ? 'bg-emerald-100 text-emerald-700' : product.status === 'borrowed' || product.status === 'ไม่ว่าง' ? 'bg-amber-100 text-amber-700' : product.status === 'maintenance' ? 'bg-red-100 text-red-700' : 'bg-gray-100 text-gray-700'}`}>
                                            {product.status === 'available' ? 'พร้อมใช้' : product.status === 'borrowed' || product.status === 'ไม่ว่าง' ? 'ถูกยืม' : product.status === 'maintenance' ? 'ส่งซ่อม' : 'เบิกแล้ว'}
                                        </span>
                                    </div>
                                    {(isSelectionMode || selectedProductIds.has(product.id!)) && <div className="absolute top-2 left-2"><div className={`w-6 h-6 rounded border flex items-center justify-center transition-colors ${selectedProductIds.has(product.id!) ? 'bg-amber-500 border-amber-500' : 'bg-white/90 border-gray-300'}`}>{selectedProductIds.has(product.id!) && <Check size={16} className="text-white" />}</div></div>}
                                </div>
                ...
                {filteredProducts.length === 0 && (searchQuery ? <EmptySearchResults query={searchQuery} /> : <EmptyInventory onAdd={() => router.push('/admin/facility/inventory/add')} />)}
            </div>
            ...
            {/* Modals */}
            {activeModal === 'detail' && selectedProduct && <FacilityProductDetailModal isOpen={true} onClose={handleCloseModal} product={selectedProduct} onAction={(action, product) => handleAction(action, product)} />}
            {isBorrowedModalOpen && <FacilityBorrowedStatusModal isOpen={true} onClose={() => setIsBorrowedModalOpen(false)} />}
            {isLogModalOpen && <LazyLogTable logs={activityLogs} title={logType === 'stock' ? 'รายงานข้อมูลอาคาร' : 'ประวัติการใช้งาน'} onClose={() => setIsLogModalOpen(false)} onGenerateReport={handleFetchLogs} isLoading={isLogLoading} />}
        </div>
    );
}
