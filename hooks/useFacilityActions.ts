...
export interface UseFacilityActionsReturn {
    handleUpdateTicket: (
...
export function useFacilityActions({ userId, userName }: UseFacilityActionsOptions): UseFacilityActionsReturn {
    ...
    const handleUpdateTicket = async (
        ...
        try {
            ...
            if (completionImage) {
                ...
                const storageRef = ref(storage, `facility_completion/${Date.now()}_${compressedImage.name}`);
                const snapshot = await uploadBytes(storageRef, compressedImage);
                completionImageUrl = await getDownloadURL(snapshot.ref);
            }

            const ticketRef = doc(db, "facility_tickets", ticket.id);
            await updateDoc(ticketRef, {
                status,
                technicianNote,
                technicianId: userId || null,
                technicianName: userName || 'Facility Technician',
                completionImage: completionImageUrl || null,
                updatedAt: serverTimestamp()
            });

            await logActivity({
                action: 'repair_update', // can reuse the same action type as it signifies ticket update
                productName: `ซ่อมอาคาร: ${ticket.room}`,
                userName: userName || "Facility Technician",
    ...
    const handleUsePart = async (
        ...
        try {
            ...
            if (signatureDataUrl) {
                ...
                const storageRef = ref(storage, `facility_signatures/${Date.now()}_sig.png`);
                const snapshot = await uploadBytes(storageRef, signatureBlob);
                signatureUrl = await getDownloadURL(snapshot.ref);
            }

            const productRef = doc(db, "facility_inventory", part.id!);
            ...
            await logActivity({
                ...
                userName: userName || "Facility Technician",
                details: `เบิกจากงานซ่อมอาคาร ${ticket.room} จำนวน ${useQuantity} ชิ้น`,
