...
interface FacilityBorrowedStatusModalProps {
    isOpen: boolean;
    onClose: () => void;
}
...
export default function FacilityBorrowedStatusModal({ isOpen, onClose }: FacilityBorrowedStatusModalProps) {
    ...
    useEffect(() => {
        ...
        setLoading(true);
        // We will assume facility transactions use the same collection but tagged or we can just filter by facility_borrow Type
        // If there's no tag yet for facility versus IT borrow, we might need to add one.
        // Assuming transactions collection contains a field `module: 'facility'` or we just use general transactions
        const q = query(
            collection(db, "facility_transactions"),
            where("type", "==", "borrow"),
            where("status", "==", "active")
        );
                        ...
                        <table className="w-full text-sm text-left">
                            <thead className="bg-gray-50 dark:bg-gray-800 text-text-secondary font-medium border-b border-border sticky top-0">
                                <tr>
                                    ...
                                    <th className="px-4 py-3">สถานที่นำไปใช้</th>
