...
interface FacilityProductDetailModalProps {
    isOpen: boolean;
    onClose: () => void;
    product: Product;
...
export default function FacilityProductDetailModal({ isOpen, onClose, product, onAction }: FacilityProductDetailModalProps) {
    ...
    useEffect(() => {
        const fetchProductHistory = async () => {
            ...
            try {
                // Assumption: Facility activities are stored in 'facility_activities' 
                // Alternatively, if they share 'activities', we filter by type or module
                // For safety, let's query 'facility_activities' based on previous patterns
                const q = query(
                ...
                // IF collection doesn't exist yet, this returns empty.
    ...
    return (
        ...
        >
            ...
            >
                ...
                <div className="relative h-48 bg-amber-100 dark:bg-gray-800 overflow-hidden">
                    {/* Blurred Background */}
                    {product.imageUrl && (
                        ...
                        <div className="flex-1 space-y-6">
                            ...
                            <div className="grid grid-cols-2 gap-y-6 gap-x-4">
                                <div className="space-y-1">
                                    <span className="text-xs text-amber-600 dark:text-amber-400 flex items-center gap-1"><Tag size={12} /> แบรนด์/ยี่ห้อ</span>
                                    <p className="font-semibold text-gray-900 dark:text-white line-clamp-1">{product.brand || "-"}</p>
                                </div>
                                <div className="space-y-1">
                                    <span className="text-xs text-amber-600 dark:text-amber-400 flex items-center gap-1"><Hash size={12} /> หมวดหมู่/Category</span>
                                    <p className="font-semibold text-gray-900 dark:text-white line-clamp-1">{product.category || "-"}</p>
                                </div>

                                {/* Serial Number */}
                                <div className="space-y-1">
                                    <span className="text-xs text-amber-600 dark:text-amber-400 flex items-center gap-1"><Hash size={12} /> Serial Number (S/N)</span>
                                    <p className="font-mono text-sm bg-amber-50 dark:bg-gray-700 px-2 py-1 rounded border border-amber-100 dark:border-gray-600 inline-block text-gray-700 dark:text-gray-200 break-all">
                                        {product.serialNumber || "-"}
                                    </p>
                                </div>

                                <div className="space-y-1">
                                    <span className="text-xs text-amber-600 dark:text-amber-400 flex items-center gap-1"><Printer size={12} /> QR Code</span>

                                    <div className="flex items-center gap-3">
                                        <div className="bg-white p-1 rounded border border-gray-100 items-center justify-center flex shrink-0">
                                            <QRCode
                                                value={`${typeof window !== 'undefined' ? window.location.origin : ''}/facility/product/${product.id || ''}`}
                                                size={48}
                                                className="w-12 h-12"
                                            />
                                        </div>
                                            ...
                                            <button
                                                onClick={async () => {
                                                    try {
                                                        ...
                                                        const url = `https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=${encodeURIComponent(`${origin}/facility/product/${product.id}`)}`;
                                ...
                                <div className="space-y-1">
                                    <span className="text-xs text-amber-600 dark:text-amber-400 flex items-center gap-1"><MapPin size={12} /> สถานที่จัดเก็บ</span>
                                    <p className="font-semibold text-gray-900 dark:text-white">{product.location || "-"}</p>
                                </div>
                                <div className="space-y-1">
                                    <span className="text-xs text-amber-600 dark:text-amber-400 flex items-center gap-1"><Package size={12} /> จำนวนคงเหลือ</span>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="bg-amber-50 dark:bg-gray-700/50 rounded-xl p-4 text-sm text-gray-600 dark:text-gray-300 space-y-2">
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="bg-amber-50 dark:bg-gray-700/50 rounded-xl p-4 text-sm text-gray-600 dark:text-gray-300 min-h-[100px] border border-amber-100 dark:border-gray-700">
                                    {product.description ? (
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="flex gap-2">
                                    ...
                                    <button
                                        ...
                                        className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-xl bg-amber-50 dark:bg-gray-700 text-amber-600 dark:text-gray-300 text-sm font-bold hover:bg-amber-100 dark:hover:bg-gray-600 transition-all border border-amber-200 dark:border-gray-600"
                                    >
                                        <Edit size={16} /> แก้ไข
                                    </button>
                                </div>
                        ...
                        <div className="space-y-3">
                            <h3 className="font-bold text-gray-900 dark:text-white flex items-center gap-2">
                                <History size={16} className="text-amber-500" /> ประวัติล่าสุด
                            </h3>
                            <div className="bg-gradient-to-br from-amber-50 to-white dark:from-gray-700/50 dark:to-gray-800/50 rounded-xl border border-amber-100 dark:border-gray-700 overflow-hidden">
                                {loadingHistory ? (
                                    ...
                                    <div className="divide-y divide-amber-100 dark:divide-gray-700">
                                        {activityHistory.slice(0, 5).map((log) => {
                                            ...
                                            return (
                                                <div key={log.id} className="flex items-start gap-3 p-3.5 hover:bg-amber-50/50 dark:hover:bg-gray-700/30 transition-colors">
