...
interface FacilityActionsBarProps {
    data: FacilityTicket[];
...
const getThaiStatus = (s: string) => {
    switch (s?.toLowerCase()) {
        ...
        case 'waiting_parts':
        case 'waiting-parts':
        case 'waitingparts': return 'รออุปกรณ์/อะไหล่';
...
export default function FacilityActionsBar({ data, onFilterChange }: FacilityActionsBarProps) {
    const { role } = useAuth();
    const [startDate, setStartDate] = useState<string>("");
    const [endDate, setEndDate] = useState<string>("");

    if (!role || !['admin', 'facility_technician', 'moderator'].includes(role)) return null;
    ...
    const prepareReportData = (tickets: FacilityTicket[]) => {
        return {
            ticketId: "FACILITY_SUMMARY",
            // ใช้ปี พ.ศ. แบบไทย
            ...
            items: tickets.map(t => {
                ...
                return {
                    code: t.id?.slice(0, 8) || '-',
                    name: `[${t.issueCategory}] ${t.description || '-'}`,
                    zone: locationDisplay,
                    status: getThaiStatus(t.status),
                    requestDate: format(addYears(dateObj, 543), 'dd/MM/yy HH:mm'),
    ...
    const handleExportExcel = () => {
        try {
            ...
            exportToExcel(data, `Facility_Report_${dateStr}`);
